...
class Vector3(object):
    ...
    def from_points(cls, p1, p2):
        ...
        ax, ay, az = p1
        bx, by, bz = p2
        v._v = [bx-ax*1.0, by-ay*1.0, bz-az*1.0]
    ...
    def get_scalar_multiplied(self, scalar):
        "An independent vector3 object based on self."
        x, y, z = self._v
        return self.from_floats(x*scalar, y*scalar, z*scalar)

    def get_vector_multiplied(self, vector):
        "An independent vector3 object based on self."
        x, y, z = self._v
        xx, yy, zz = vector
    ...
    def get_vector_divided(self, vector):
        "An independent vector based on self."
        x, y, z = self._v
        xx, yy, zz = vector
        return self.from_floats(x / xx, y / yy, z / zz)
    ...
    def scale(self, scale):
        """Scales the vector by another vector or a scalar. Same as the
        *= operator.
    ...
    def get_normalised(self):
        "Makes an independent vector based on self and of length 1."
        x, y, z = self._v
        l = sqrt(x*x + y*y + z*z)
    ...
    def get_projected(self, planenormal):
        """
        Use this to find a projection of self on a plane.

        planenormal -- Vector3 of UNIT LENGTH which is normal to the plane.

        """

        d = self.dot( planenormal )
        v = self + ( planenormal * -d )     
        return v
    ...
    def get_projected_line(self, direction):
        ...
        factor = dp / (direction.dot(direction))
    ...
    def cross(self, other):
        ...
        """Returns the cross product of this vector with another as a new independent vector.
...
def project_vec3( vector, planenormal ):
    """
    Use this to find a projection of vector3 on a plane.
    'vector' Vector3 type object.
    'planenormal' Vector3 type of UNIT LENGTH which is normal to the plane.

    """
    nx, ny, nz = planenormal
    x, y, z = vector
    d = abs(nx*x + ny*y + nz*z)          # use abs() because the vec we add to vector needs to always point in the direction of the plane
    v = vector + (planenormal * -d)
    return v
...
if __name__ == "__main__":
    ...
    print
    print
    v1 = Vector3(4.0, 0.0, 2).get_normalised()
    print v1
    print
